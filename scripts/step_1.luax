--[[
	...
	5) Make sure the SkeletalMeshComponent is facing forward when your hand is facing forward. If it is not then
		you will have to make an extra adjustment on line 42 using the correct values for your specific game until the mesh is facing forward. 
	6) While keeping your hand steady, move your head around and see if the SkeletalMeshComponent attached to your hand appears to shift slightly, even
		...
		before hands will work properly. TOW does have this issue. If you are running this tutorial in TOW, uncomment line 46 to correct 
...
function on_lazy_poll()
	if not hands.exists() then
		...
		--uevrUtils.fixMeshFOV(hands.getHandComponent(1), "ForegroundPriorityEnabled", 0.0, true, true, true)
	end
end
